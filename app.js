...
// ⭐ REBIRTH UI
const rebirthCountEl =
    document.getElementById("rebirthCount");

const naturePointsEl =
    document.getElementById("naturePoints");

const rebirthRequirementEl =
...
if (rebirthCountEl) {
    rebirthCountEl.textContent =
        rebirths;
}

if (naturePointsEl) {
    naturePointsEl.textContent =
        naturePoints;
}

if (rebirthRequirementEl) {
    rebirthRequirementEl.textContent =
        rebirthRequirement;
}

// Enable rebirth when enough focus time
// has been completed.
if (rebirthBtn) {

    rebirthBtn.disabled =
        focusSeconds < rebirthRequirement * 60;
...
function performRebirth() {
    ...
    // Make sure the player has enough focus time.
    if (
        focusSeconds <
        rebirthRequirement * 60
    ) {
    ...
    // Make sure the player isn't currently focusing.
    if (growing) {

        showNotification(
            "🛑 Stop your focus session before rebirthing!"
        );
    ...
    const confirmed =
        confirm(
            "🌟 Rebirth your forest?\n\n" +
            "Your Wood, Iron, forest and growth upgrades " +
            "will reset.\n\n" +
    ...
    // Save the new rebirth state.
    saveGame();


    showNotification(
        `🌟 Rebirth ${rebirths} complete! +1 Nature Point`
    );
